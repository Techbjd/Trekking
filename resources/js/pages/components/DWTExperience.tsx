...
const DWTExperience: React.FC = () => {
  return (
    <section className="relative bg-white w-360 h-125">
      {/* Title */}
      <h2 className="absolute text-black text-center w-67.25 left-146.25 top-10 font-['DM_Sans'] font-bold text-[32px] leading-10.5 m-0">
        DWT Experience
      </h2>

      {/* Description */}
      <p className="absolute text-[#666666] text-center w-170.25 left-97.25 top-22.5 font-['DM_Sans'] font-medium text-[14px] leading-4.5 m-0">
      ...
      {/* Cards */}
      {cards.map((card, i) => (
        <div key={i} className={`absolute ${card.width} ${card.left} top-39`}>
      ...
      {/* Left Arrow */}
      <div className="absolute flex items-center justify-center rounded-full bg-white w-9.25 h-9.25 left-38.5 top-59.5 shadow-[0px_2px_4px_rgba(0,0,0,0.1)]">
        <ChevronLeft className="w-6 h-6 text-black" />
      </div>

      {/* Right Arrow */}
      <div className="absolute flex items-center justify-center rounded-full bg-white w-9.25 h-9.25 left-313.25 top-59.5 shadow-[0px_2px_4px_rgba(0,0,0,0.1)]">
